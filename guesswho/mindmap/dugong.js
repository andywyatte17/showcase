...
Dugong.Diagram = function(elem){
  ...
  this.svg.setAttributeNS(null,"class", "dugong");
...
Dugong.Node = function(name) {
  ...
  foDiv.style.display = "inline-block";
  foDiv.style.minWidth = "100%";
  ...
  this.resize = function() {
    ...
    var currentWidth = currentHeight * Dugong.boxRatio;
    while (true) {
      fObject.setAttributeNS(null,"height", currentHeight - Dugong.boxMargin*2);
      fObject.setAttributeNS(null,"width", currentWidth - Dugong.boxMargin*2);

      if (div.offsetHeight > (currentHeight - Dugong.boxMargin*2) ||
          div.offsetWidth > (currentWidth - Dugong.boxMargin*2)) {
        currentHeight += 1;
        currentWidth = currentHeight * Dugong.boxRatio;
      }
      else
        break;
    }
...
Dugong.populate = function(klass) {
  window.addEventListener("load", function(){
    ...
    for (var i=0;i<dugongElements.length;i++) {
      var thisElem = dugongElements[i];
      if (thisElem.tagName == "svg") continue;

      var diagram = new Dugong.Diagram(thisElem);
      diagram.resize();
